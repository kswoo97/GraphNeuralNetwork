...
class benzene_NNConv(torch.nn.Module) :

    def __init__(self, dataset, gconv = NNConv, device = "cuda", latent_dim = [16, 16, 16]) :
        super(benzene_NNConv, self).__init__()
        self.linears = torch.nn.ModuleList()
        self.convs = torch.nn.ModuleList()
        self.device = device
        self.latent = latent_dim

        self.linears.append(torch.nn.Linear(5, dataset.num_features*latent_dim[0]))
        for i in range(0, (len(latent_dim) - 1)):
            self.linears.append(torch.nn.Linear(5, latent_dim[i]*latent_dim[i+1]))

        self.convs.append(
            gconv(in_channels=dataset.num_features,
                  out_channels=latent_dim[0], nn = self.linears[0])
        )
        for i in range(0, (len(latent_dim) - 1)) :
    ...
    def forward(self, data, training_with_batch) :
        ...
        if training_with_batch :
            ...
            # batchwise computing
            self.outs = torch.zeros(self.latent[-1],
                                    dtype = float, requires_grad=True, device = self.device).reshape(1,-1)
